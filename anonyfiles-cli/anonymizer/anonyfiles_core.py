...
from .csv_processor import CsvProcessor # <--- DÉCOMMENTEZ CETTE LIGNE
...
# Laisser PROCESSOR_MAP tel quel, il sera toujours utilisé pour sélectionner le bon processeur
PROCESSOR_MAP = { #
    ".txt": TxtProcessor,
    # ".csv": CsvProcessor,
    # ".docx": DocxProcessor,
...
class AnonyfilesEngine:
    def __init__(self, config: Dict[str, Any], 
                 ...
                 custom_replacement_rules: Optional[List[Dict[str, str]]] = None): # NOUVEAU paramètre
        ...
        self.custom_rules = custom_replacement_rules or [] # Stocker les règles personnalisées
    ...
    def _apply_custom_rules_to_block(self, text_block: str) -> str:
        ...
        for rule in self.custom_rules:
            ...
            replacement = rule.get("replacement", "[CUSTOM_REDACTED]") # Remplacement par défaut
            if pattern: # S'assurer qu'il y a un pattern
                try:
                    # On suppose que 'pattern' est une regex. 
                    # Pour une chaîne littérale, il faudrait `re.escape(pattern)`
    ...
    def anonymize(self, input_path: Path, output_path: Optional[Path], 
                  entities: Optional[List[str]], # Filtre pour les types d'entités spaCy
                  dry_run: bool, 
                  log_entities_path: Optional[Path], 
                  mapping_output_path: Optional[Path],
                  **kwargs): # Pour options spécifiques aux processeurs (ex: has_header pour CSV)
        ...
        # Gérer les kwargs spécifiques comme 'has_header' pour CsvProcessor si nécessaire
        extract_kwargs = {}
        ...
        if not any(block.strip() for block in blocks_after_custom_rules): #
            # ... (Gestion des fichiers vides comme dans votre code original) ...
            # Note: La méthode `write_final_blocks` de TxtProcessor devrait gérer l'écriture d'un fichier vide.
            if not dry_run and output_path: #
                output_path.parent.mkdir(parents=True, exist_ok=True) #
                if isinstance(processor, TxtProcessor): #
                    # Assurez-vous que TxtProcessor a une méthode write_final_blocks
        ...
        for block_text_processed in blocks_after_custom_rules: # Utiliser les blocs modifiés
        ...
        # ... (Filtrage de unique_spacy_entities comme dans votre code original) ...
        ...
        if not unique_spacy_entities:
            if not dry_run and output_path: #
                output_path.parent.mkdir(parents=True, exist_ok=True) #
                if isinstance(processor, TxtProcessor): #
                    # Écrire les blocs qui ont potentiellement eu les modifs custom
                    processor.write_final_blocks(output_path, blocks_after_custom_rules)
                else: # Pour les autres types, les règles custom n'ont pas été appliquées
                      # donc blocks_after_custom_rules == original_blocks. On copie l'original.
                    import shutil
                    shutil.copyfile(input_path, output_path)
            # ... (génération mapping vide) ...
            if mapping_output_path: #
                mapping_output_path.parent.mkdir(parents=True, exist_ok=True) #
                with open(mapping_output_path, "w", encoding="utf-8") as f: #
                    f.write("Code,Type,Nom Original\n") #
        ...
        for i, block_text_after_custom in enumerate(blocks_after_custom_rules):
            if block_text_after_custom.strip() and spacy_entities_per_block_with_offsets[i]:
                # Appliquer les remplacements spaCy
                fully_anonymized_block = apply_positional_replacements(
                    block_text_after_custom,
                    replacements_map_spacy, # Remplacements spaCy
                    spacy_entities_per_block_with_offsets[i] # Offsets spaCy dans ce bloc
                )
                final_anonymized_blocks.append(fully_anonymized_block)
            else: # Pas d'entités spaCy dans ce bloc, ou bloc vide
        ...
        if not dry_run and output_path: #
            output_path.parent.mkdir(parents=True, exist_ok=True) #
            if isinstance(processor, TxtProcessor): #
                # Le TxtProcessor doit avoir une méthode pour écrire ces blocs finaux
                processor.write_final_blocks(output_path, final_anonymized_blocks)
            else:
                # Pour les autres types de fichiers (non-TXT), les règles custom n'ont pas été appliquées
                # par l'engine. Donc `final_anonymized_blocks` est le résultat de l'application
                # des remplacements spaCy sur les `original_blocks`.
                # La méthode `replace_entities` existante est appropriée ici.
                # Les `spacy_entities_per_block_with_offsets` sont corrects car ils ont été calculés
                # sur `original_blocks` (puisque `blocks_after_custom_rules == original_blocks` pour non-TXT).
        ...
        # 6. Logging et Mappings (basés sur les entités spaCy)
        # ... (votre code de logging et de création de mapping reste le même, utilisant unique_spacy_entities et mapping_dict_spacy) ...
        ...
        return { #
            "status": "success", #
            "entities_detected": unique_spacy_entities, # Entités spaCy détectées après règles custom
            "output_path": str(output_path) if output_path else None, #
            "replacements": replacements_map_spacy, # Remplacements spaCy #
        }