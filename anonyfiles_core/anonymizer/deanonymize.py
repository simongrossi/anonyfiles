    ...
    def _load_mapping(self):
        try:
            with open(self.mapping_path, newline="", encoding="utf-8") as csvfile:
                ...
                # possible_label_fields and possible_source_fields removed as unused.
                header_fields = reader.fieldnames
                if not header_fields:
                    self.map_loading_warnings.append(
                        f"Fichier mapping vide ou sans en-tête : {self.mapping_path}"
                    )
                    return

                code_field = next(
                    (f for f in possible_code_fields if f in header_fields), None
                )
                original_field = next(
                    (f for f in possible_original_fields if f in header_fields), None
                )
                ...
                for row_number, row in enumerate(reader, 1):
                    try:
                        ...
                        if anonymized_val is not None and original_val is not None:
                            self.code_to_originals[anonymized_val.strip()].add(
                                original_val.strip()
                            )
                        else:
                            self.map_loading_warnings.append(
            ...
            self.map_loading_warnings.append(
                f"Fichier mapping non trouvé : {self.mapping_path}"
            )
    ...
    def deanonymize_text(self, text: str, dry_run: bool = False) -> Tuple[str, dict]:
        ...
        if self.code_to_originals or not self.map_loading_warnings:
            ...
            self.warnings.append(
                "Désanonymisation ignorée car le mapping n'a pas pu être chargé ou est vide."
            )

        codes_in_mapping_that_were_referenced = {
            k for k in self.code_to_originals if k in all_distinct_codes_found_in_text
        }
        ...
        report = {
            ...
            "replacements_ambiguous_kept_count": attempted_replacements_info[
                "ambiguous_kept"
            ],
    ...
    def generate_report(self, report: dict, output_path: Optional[str] = None):
        ...
        if map_collisions_display:
            ...
            for code, originals_set in map_collisions_display.items():
                lines.append(
                    f"  - Code '{code}' peut correspondre à : {list(originals_set)}"
                )
        ...
        if report.get("warnings_generated_during_deanonymization"):
            ...
            for idx, warning_msg in enumerate(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print(
            "Usage: python deanonymize.py <input_file_path> <mapping_csv_path> <output_file_path> [--dry-run] [--strict | --permissive]"
        )
        sys.exit(1)
    ...
    print(
        f"Lancement désanonymisation via CLI: Entrée='{cli_input_file_path}', Mapping='{cli_mapping_csv_path}', Sortie='{cli_output_file_path}'"
    )
    print(f"Options: dry-run={cli_is_dry_run_mode}, strict={cli_is_strict_mode_active}")

    try:
        cli_deanonymizer_instance = Deanonymizer(
            cli_mapping_csv_path, strict=cli_is_strict_mode_active
        )

        if (
            cli_deanonymizer_instance.map_loading_warnings
            and not cli_deanonymizer_instance.code_to_originals
        ):
            print(
        ...
        cli_restored_text_content, cli_report_information = (
            cli_deanonymizer_instance.deanonymize_text(
                cli_source_text_content, dry_run=cli_is_dry_run_mode
            )
        )
        ...
        print(
            f"Erreur (CLI): Problème avec les colonnes du fichier mapping - {e_key_cli}"
        )
