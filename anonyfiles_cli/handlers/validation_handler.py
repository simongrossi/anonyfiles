...
from ..exceptions import (
    ConfigurationError,
)  # Assurez-vous que cette exception est définie
...
class ValidationHandler:
    ...
    @classmethod
    def validate_deanonymize_inputs(cls, input_file: Path, mapping_csv: Path):
        ...
        if not input_file.exists() or not input_file.is_file():
            raise ConfigurationError(
                f"Le fichier d'entrée '{input_file}' est introuvable ou n'est pas un fichier."
            )
        if not mapping_csv.exists() or not mapping_csv.is_file():
            raise ConfigurationError(
                f"Le fichier de mapping '{mapping_csv}' est introuvable ou n'est pas un fichier."
            )
        if mapping_csv.suffix.lower() != ".csv":
            raise ConfigurationError(
                f"Le fichier de mapping doit être un fichier CSV (.csv), mais a l'extension : {mapping_csv.suffix}"
            )
    ...
    def _validate_file_extension(cls, file_path: Path):
        ...
        if file_path.suffix.lower() not in cls.SUPPORTED_EXTENSIONS:
            raise ConfigurationError(
                f"Extension de fichier non supportée : {file_path.suffix}. Extensions supportées : {', '.join(cls.SUPPORTED_EXTENSIONS)}"
            )
    ...
    def _validate_csv_options(
        ...
        if csv_no_header and input_file.suffix.lower() != ".csv":
            raise ConfigurationError(
                "--csv-no-header ne peut être utilisé que pour les fichiers CSV."
            )
        ...
        if custom_replacements_json:
            try:
                ...
                if not isinstance(parsed, list):
                    raise ConfigurationError(
                        "Les règles de remplacement personnalisées doivent être une liste JSON."
                    )
                for i, rule in enumerate(parsed):
                    if (
                        not isinstance(rule, dict)
                        or "pattern" not in rule
                        or "replacement" not in rule
                    ):
                        raise ConfigurationError(
                            f"Règle personnalisée invalide à l'index {i}. Chaque règle doit être un objet JSON avec 'pattern' et 'replacement'."
                        )
            except json.JSONDecodeError as e:
                raise ConfigurationError(
                    f"Format JSON invalide pour les règles de remplacement personnalisées : {e}."
                )
            except Exception as e:
                raise ConfigurationError(
                    f"Erreur inattendue lors de la validation des règles personnalisées : {e}."
                )
