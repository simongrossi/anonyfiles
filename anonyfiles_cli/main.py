...
app = typer.Typer(
    ...
    help="Anonyfiles CLI - Outil d'anonymisation de documents.",
)
console = ConsoleDisplay()

# Ajout des sous-commandes depuis les modules séparés
app.add_typer(
    anonymize.app, name="anonymize", help="Commandes pour anonymiser les fichiers."
)
app.add_typer(
    deanonymize.app,
    name="deanonymize",
    help="Commandes pour desanonymiser les fichiers.",
)
...
def main_callback(
    ...
    if not user_config_path.exists():
        console.console.print(
            f"[dim]ℹ️ Fichier de configuration utilisateur non trouvé. Création d'une configuration par défaut à : {user_config_path}[/dim]"
        )
        try:
            ConfigManager.create_default_user_config()
        except Exception as e:
            console.console.print(
                f"[bold red]❌ Erreur lors de la création de la configuration par défaut au démarrage : {e}[/bold red]",
                style="red",
            )
