...
@app.command(
    name="process", help="Anonymise un fichier en fonction des options fournies."
)
def process_anonymize(
    input_file: Path = typer.Argument(
        ...,
        help="Fichier à anonymiser",
        exists=True,
        file_okay=True,
        dir_okay=False,
        readable=True,
    ...
    config: Optional[Path] = typer.Option(
        ...
        "--config",
        "-c",
        help="Fichier de configuration YAML (optionnel). S'il est fourni, il doit exister.",
        exists=True,
        file_okay=True,
        dir_okay=False,
        readable=True,
    ...
    bundle: Optional[Path] = typer.Option(
        ...
        "--bundle",
        "--output-bundle",
        help="Chemin du fichier zip regroupant toutes les sorties (optionnel).",
    ),
    output_dir: Path = typer.Option(
        Path("."),
        "--output-dir",
        help="Dossier où écrire les fichiers de sortie par défaut (si les chemins spécifiques ne sont pas fournis).",
        file_okay=False,
        dir_okay=True,
        writable=True,
        resolve_path=True,
    ...
    append_timestamp: bool = typer.Option(
        True, help="Ajoute un timestamp aux noms des fichiers de sortie par défaut."
    ),
    force: bool = typer.Option(
        False,
        "--force",
        "-f",
        help="Force l’écrasement des fichiers de sortie existants.",
    ),
):
    """
    ...
    try:
        ...
        if interactive and not exclude_entities:
            ...
            console.console.print(
                "[yellow]--interactive ignoré car --exclude-entities est déjà fourni.[/yellow]"
            )
