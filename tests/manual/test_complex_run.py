...
def run_test():
    ...
    with open(MAPPING_FILE, "r", encoding="utf-8", newline="") as f:
        ...
        try:
            next(
                reader
            )  # Skip header if present (Engine usually puts "Original,Replacement")
    ...
    for city in cities:
        if city in mapping_dict:
            ...
            print(
                f"WARNING: City '{city}' not found in mapping (maybe not detected by SpaCy?)."
            )
    ...
    if person in mapping_dict:
        ...
        if count_in_text >= 2:
            ...
            print(
                "WARNING: Token appeared less than expected (maybe text was detected differently each time?)"
            )
    else:
