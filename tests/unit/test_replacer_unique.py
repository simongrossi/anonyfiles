...
import pytest
from anonyfiles_core.anonymizer.replacer import ReplacementSession, _GENERATOR_REGISTRY
...
def test_replacer_uniqueness_for_static_types():
    ...
    replacement_rules = {
        "LOC": {"type": "redact", "options": {"text": "[LOC_MASQUE]"}}
    }
    
    replacements, mapping = session.generate_replacements(unique_spacy_entities, replacement_rules)
