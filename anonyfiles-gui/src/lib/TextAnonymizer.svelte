<script lang="ts">
  ...
  // Split view logic
  let showSplitView = true; // true = côte à côte, false = mono-vue
  let showOriginal = false; // false = anonymisé, true = original
</script>
...
{#if outputText}
  <div class="border border-green-200 bg-green-50 text-green-900 rounded-2xl p-4 flex flex-col gap-2 mt-4 shadow-sm">
    <div class="flex items-center gap-4 mb-2">
      <span class="font-bold text-green-900 text-lg flex-1">
        Aperçu {showSplitView ? "Avant / Après" : (showOriginal ? "Original" : "Anonymisé")}
      </span>
      <button
        class="px-4 py-1 rounded-md bg-zinc-200 hover:bg-zinc-300 font-medium text-zinc-800 transition mr-2"
    ...
    {#if showSplitView}
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="flex flex-col">
          <span class="mb-1 font-semibold text-zinc-600">Texte original</span>
          <textarea
            class="bg-white text-zinc-900 p-3 rounded-xl resize-y min-h-[60px] border border-zinc-300 font-mono w-full"
            readonly
            value={inputText}
            rows="6"
          />
        ...
        <div class="flex flex-col">
          <span class="mb-1 font-semibold text-green-800">Texte anonymisé</span>
          <textarea
    ...
    {:else}
      <div class="flex flex-col gap-2">
        <span class="mb-1 font-semibold">{showOriginal ? "Texte original" : "Texte anonymisé"}</span>
        <textarea
          class={showOriginal
            ? "bg-white text-zinc-900 p-3 rounded-xl resize-y min-h-[60px] border border-zinc-300 font-mono w-full"
            : "bg-green-50 text-green-900 p-3 rounded-xl resize-y min-h-[60px] border border-green-200 font-mono w-full"}
          readonly
