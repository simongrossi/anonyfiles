...
_request_context_path: ContextVar[Optional[str]] = ContextVar("request_path", default=None)
_request_context_ip: ContextVar[Optional[str]] = ContextVar("request_ip", default=None)

# --- Constantes ---
CONFIG_TEMPLATE_PATH = Path(__file__).resolve().parent.parent / "anonyfiles_core" / "config" / "config.yaml"
JOBS_DIR = Path("jobs")
...
class AppConfig(BaseSettings):
    ...
    spacy_model: str = Field(default="fr_core_news_md", description="Modèle spaCy par défaut")
...
class YamlConfigSettingsSource(PydanticBaseSettingsSource):
    ...
    def __call__(self) -> Dict[str, Any]:
        if not self.yaml_file.is_file():
            logger.warning(f"Fichier de configuration YAML non trouvé : {self.yaml_file}")
            return {}
        ...
        try:
            ...
            if not isinstance(config_data, dict):
                logger.warning(f"Le fichier {self.yaml_file} ne contient pas un dictionnaire valide.")
                return {}
