...
class Job:
    ...
    def get_file_path_sync(self, file_key: str) -> Optional[Path]:
        if file_key == "output":
            ...
            return self._find_latest_file_sync("_mapping_*.csv")
        elif file_key == "log_entities":
            return self._find_latest_file_sync("_entities_*.csv")
    ...
    def delete_job_directory_sync(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Tâche {self.job_id}: Répertoire {self.job_dir} supprimé avec succès."
            )
            return True
        except OSError as e:
            logger.error(
    ...
    async def delete_job_directory_async(self) -> bool:
        ...
        try:
            ...
            paths = await run_in_threadpool(
                lambda: sorted(
                    self.job_dir.rglob("*"), key=lambda p: len(str(p)), reverse=True
                )
            )
            for p in paths:
